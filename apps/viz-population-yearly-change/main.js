...
function renderStacked(rows, years, layout) {
  ...
  const margin = {
    top: Math.max(16, Math.min(32, height * 0.11)),
    right: marginRight,
  ...
  g.selectAll('text.total-label')
    ...
    .attr('y', (d) => y(d.total_population / 10000) - 5)
    .attr('text-anchor', 'middle')
    .call(applySvgTextStyle)
    .attr('opacity', (d) => {
      const ay = activeYear()
...
function renderGrid(rows, years, zips, zipNameByZip, layout) {
  ...
  yAxisG.selectAll('.tick text').each(function (zip) {
    ...
    const alias = shortZipAlias(zipNameByZip.get(String(zip)))
    text.text(null)
    text
      .append('tspan')
      .attr('x', -9)
